...
func TestObjectsAreEqualValues(t *testing.T) {
	thing := "compare this thing to itself"
	if !ObjectsAreEqualValues(thing, thing) {
		t.Error("ObjectsAreEqualValues() on two references to the same object should return true")
	}
}

// func TestEqualValues(t *testing.T) {
// 	if EqualValues(t, "value", "different value", "\"value\" != \"different value\"") {
// 		t.Errorf("EqualValues() should say \"value\" != \"different value\"")
// 	}
// }
...
func Test_realRuntimeCaller(t *testing.T) {
	_, _, _, ok := realRuntime{}.Caller(0)
	if !ok {
		t.Error("realRuntime.Caller() should succeed")
	}
}

type badCallerRuntime struct{}

func (badCallerRuntime) Caller(skip int) (pc uintptr, file string, line int, ok bool) {
	return
}

func (badCallerRuntime) FuncForPC(pc uintptr) (f *runtime.Func) {
	return
}

func Test_callerInfoBadCaller(t *testing.T) {
	c := callerInfoStruct{runtime: badCallerRuntime{}, testing: true}
	retVal := c.callerInfo()
	if retVal != nil {
		t.Error("callerInfo() should have returned nil when Caller() returned !ok")
	}
}

type badFuncForPCRuntime struct{}

func (badFuncForPCRuntime) Caller(skip int) (pc uintptr, file string, line int, ok bool) {
	return runtime.Caller(skip)
}

func (badFuncForPCRuntime) FuncForPC(pc uintptr) (f *runtime.Func) {
	return nil
}

func Test_callerInfoBadFuncForPC(t *testing.T) {
	c := callerInfoStruct{runtime: badFuncForPCRuntime{}}
	retVal := c.callerInfo()
	if retVal == nil {
		t.Error("callerInfo() with bad FuncForPC() should not have returned nil")
	}
}

func Test_getWhitespaceString(t *testing.T) {
	g := getWhiteSpaceSringStruct{runtime: badCallerRuntime{}, testing: true}
	if g.getWhitespaceString() != "" {
		t.Error("getWhitespaceString() should return \"\" when Caller() fails")
	}
}

func Test_isTest(t *testing.T) {
	if !isTest("four", "four") {
		t.Error("isTest() should return true when its arguments have the same length")
	}
}

type isTestRuntime struct{}

func (isTestRuntime) FuncForPC(pc uintptr) (f *runtime.Func) {
	return runtime.FuncForPC(pc)
}

func ExampleCall_callerInfo() (s []string) {
	s = callerInfoStruct{runtime: realRuntime{}}.callerInfo()
	if s == nil {
		return nil
	}
	return
...
func Test_callerInfo_isTest(t *testing.T) {
	s := ExampleCall_callerInfo()
	if s == nil {
		t.Error("Should have succeeded getting caller info")
	}
}
